const plugin = {
  ...
  async renderEmbed(app) {
    ...
    async function fetchNotesPromise(tags) {
      let commaSeparatedTags = tags.reduce((result, tag) => { return tag + "," + result }, "");
      return app.filterNotes({ tag: commaSeparatedTags });
    }
    ...
    function fetchAndLoadNotes(userDefinedTags, debounce) {
      ...
      fetchNotesPromise(["daily-jots"]).then(
        (notes) => {
    ...
    fetchAndLoadNotes("daily-jots", pluginState.isFetchHappening);
