const plugin = {
  ...
  getWeek: (date) => {
    ...
    //TODO format divs according to weekly jot existence and tasks
    const weekCountSpan = document.createElement("span");
    weekCountSpan.className = "flatpickr-day weekly";
    weekCountSpan.append(getMaterialIcon("priority", "event"));
  ...
  async onEmbedCall(app, callType, date, shiftPressed, dayName) {
    ...
    function getNotesWithWeeks(notes) {
      ...
      const dateFormatRegex = /^Week (?<week>\b[1-9]{1,2}) of (?<year>\b\d{4}\b)/gm;
        ...
        if (weekNote) {
          ...
          await app.createNote(dayName, currentTagsArray)
            .then((uuid) => {
              ...
              this._state.weekJots.push({ year, week, uuid, hasTasks: pluginConstants.hasTasks.NONE, type: pluginConstants.jotTypes.WEEK });
            });
        }
        break;
      case "navigate-month":
...
<body>
  ...
  <script>
      ...
      window.callAmplenotePlugin("settings").then(({ status, firstDayOfWeek, tags }) => {
        ...
        switch(status) {
          case "${changed.TAGS}":
            ...
            fetchNotes(true, true, fpInstance);
            break;
          case "${changed.FDOW}":
    ...
    function fetchNotes(ribbonNeedsRefresh, tableNeedsRedraw, fpInstance) {
      //TODO figure out when fpInstance is null
      const thisYearDate = !fpInstance ? new Date() : new Date(fpInstance.currentYear, fpInstance.currentMonth);
      window.callAmplenotePlugin("fetch", thisYearDate).then(() => {
        ...
        if (tableNeedsRedraw && !!fpInstance) fpInstance.redraw();
        setupWeeklyNotes(fpInstance);
      });
    }
        ...
        element.addEventListener('click', () => 
          window.callAmplenotePlugin("navigate-week", currentDate, shiftPressed, weekNumber)
            .then(() => {
              ...
              //fpInstance.clear();
              //fpInstance.jumpToDate(currentDate, false);
            })
        );
      });
