const plugin = {
  ...
  async onEmbedCall(app, callType, date, shiftPressed, dayName) {
    ...
    function getMonthJot(jotDate) {
      const isMonthlyNote = (jot) => (jot.type === pluginConstants.jotTypes.WEEK);
      const matchesMonth = (jot) => (jotDate.getMonth() == date.getMonth() && jotDate.getFullYear() == jot.year);
      return pluginState.weekJots.find((n) => (isMonthlyNote(n) && matchesMonth(n)));
    }
      ...
      case "fetch":
        ...
        return Promise.all([
          ...
          fetchMonthNotesPromise(userTags, date.getFullYear()).then(
            (notes) => {
              console.log("fetched monthly jots", notes);
              const simpleNotes = [...getNotesWithMonths(notes)];
              console.log("filtered monthly jots", simpleNotes);
              checkForTasksPromises(simpleNotes).then((notesWithTaskStatus) => {
                console.log("GOT for month fetch", notesWithTaskStatus);
                pluginState.monthJots = [];
                notesWithTaskStatus.forEach((note) => pluginState.monthJots.push(note));
                console.log("Now monthJots be like", pluginState.monthJots);
              });
            }
          )
        ])
  ...
  <script>
    ...
    function setupWeeklyNotes(fpInstance) {
      ...
      months.forEach((element) => {
        ...
        const monthName = element.textContent;

        window.callAmplenotePlugin("draw-month", currentDate, shiftPressed)
          .then((hasJot) => {
            if (!hasJot) element.className += " no-jot";
        });

        //attach the click handler to navigate/create to the content of the note
        element.addEventListener('click', () => {
          const clickedMonthDate = fpInstance.parseDate(monthName + fpInstance.currentYear, "FY");
          console.log("clicked month date is", clickedMonthDate);
          window.callAmplenotePlugin("navigate-month", clickedMonthDate, shiftPressed)
            .then(() => {
              fpInstance.selectedDates = [];
              redrawCalendar(fpInstance);
            })
