const plugin = {
  ...
  async onEmbedCall(app, callType, date, shiftPressed, dayName) {
    ...
    //Maybe implement a switch on the callType parameter (settings/create)
    if (callType === "create") {
  ...
  async renderEmbed(app) {
    ...
    //TODO either implement this function, or remove it
    //debounceRenderEmbedFunctionCall(fetchAndLoadNotes, [userTags]);

    /**
     * This debounce function is required because renderEmbed is being called twice 
     * when first calling the plugin from the appOption menu, 
     * when re-opening the side panel, 
     * when collapsing and expanding the plugin note.
     * Fetching becomes twice as expensive. Therefore this workaround is needed.
     * I have reported this bug, waiting for an update (03/17/2025)
     */
    function debounceRenderEmbedFunctionCall(funcToDebounce, args, callbackFunc) {
      if (pluginState.isFetchHappening) {
        //console.log("Leaving fetch function execution.");
        return;
      }
