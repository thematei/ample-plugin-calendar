(() => {
  ...
  function filterNotesWithDates(notes) {
    console.log("FilterNotesWithDates");
  ...
  function filterNotesWithWeeks(notes) {
    console.log("FilterNotesWithWeeks");
    const jotType = pluginConstants.jotTypes;
  ...
  function filterNotesWithMonths(notes) {
    console.log("FilterNotesWithMonths");
  ...
  function getMonthlyButtonModel(date) {
    ...
    return utils.getElementModel("div", "monthlyButton", "smol-button notepad", [buttonGraphicsContainer]);
  }
  function getRefreshButtonModel() {
    return utils.getElementModel("div", "refreshButton", "smol-button refresh", [utils.getMaterialIcon("refresh")]);
  }
  ...
  function getSettingsButtonModel() {
    return utils.getElementModel("div", "settingsButton", "smol-button settings", [utils.getMaterialIcon("settings")]);
  }
  function getWeeklyButtonsModel(date) {
    ...
    const newWeekElementsModels = displayedWeekNumbers.map((weekNo) => {
      const weekElementModel = utils.getElementModel("span", undefined, "flatpickr-day", [weekNo]);
  ...
  function getRibbon(userTags) {
    console.log("Got to ribbon");
    ...
    //Sums the length of all the jot lists from the state
    const sumOfAllJotCounts = ["jots", "weekJots", "monthJots"].reduce((acc, cur) => (pluginState[cur].length + acc), 0);
    //Generates the jots counter model
    const jotCountModel = utils.getElementModel("div", "jot-count", "jot-count", [sumOfAllJotCounts, utils.getMaterialIcon("description")]);
    //Appends the tag list and jots counter models to the ribbon container model
    const ribbonElementModel = utils.getElementModel("div", "ribbon", "ribbon", [userTagDisplayModel, jotCountModel]);
  ...
  var helpers = () => {
    ...
    function newHTMLElement({ tagName, id, className, children }, onClickHandler) {
      ...
      if (onClickHandler) newElement.addEventListener("click", (...args) => onClickHandler(...args));
      return newElement;
    };
    ...
    function navigateToWeeklyJot(fpInstance, weekNumber) {
      ...
      console.log("weekly navigating", weekNumber);
    ...
    function handleSettings(fpInstance) {
      window.callAmplenotePlugin("settings").then(({ status, statusOptions, firstDayOfWeek, tags }) => {
        const changed = statusOptions;
        switch (status) {
          case changed.TAGS:
    ...
    function redrawCalendar(fpInstance) {
      ...
      window.callAmplenotePlugin("refresh", displayedMonthsDate).then(({ monthlyButton, refreshButton, todayButton, settingsButton, weeklyButtons, ribbon }) => {
        fpInstance.redraw();

        replaceHTMLElement(".smol-button.notepad", ".flatpickr-current-month", () => {
          return newHTMLElement(monthlyButton, () => navigateToMonthlyJot(fpInstance));
        }, prepend);

        replaceHTMLElement("#refreshButton", ".flatpickr-current-month", () => {
          return newHTMLElement(refreshButton, () => fetchNotes(fpInstance));
        }, prepend);
        ...
        replaceHTMLElement("#settingsButton", ".flatpickr-current-month", () => {
          return newHTMLElement(settingsButton, () => handleSettings(fpInstance));
        }, append);

        replaceHTMLElement(".flatpickr-weeks", ".flatpickr-weekwrapper", () => {
          return newHTMLElement(weeklyButtons, (event) => {
            navigateToWeeklyJot(fpInstance, event.target.textContent);
          });
        }, append);

        replaceHTMLElement("#ribbon", ".calendar-container", () => {
          return newHTMLElement(ribbon, () => {
            /*TODO only apply this onClickHandler to child elements*/
  ...
  var handlers = {
    ...
    onDayCreate: (_, __, ___, dayElem) => {
      ...
      window.callAmplenotePlugin("draw", dayElem.dateObj)
        .then((groomedElement) => {
          dayElem.className = `${dayElem.className}${groomedElement.className}`;
          groomedElement.children.forEach((childElement) => dayElem.append(newHTMLElement(childElement)));
        });
    },
  }
  ...
  var plugin = {
    ...
    async onEmbedCall(app, callType, date, shiftPressed, dayName, elements) {
      //TODO remove the elements parameter, it doesn't work
      ...
      switch (callType) {
        case "settings":
          ...
          if (userSettings) {
            ...
            const statusOptions = changed;

            return updateSettingsPromise(app, firstDayOfWeek, tags).then(() => {

              this._state.fdow = currentFDOW;

              return Promise.resolve({ status, statusOptions, firstDayOfWeek, tags });
            });
          } else {
            return { status: changed.NONE, statusOptions };
          }
        case "fetch":
        ...
        case "refresh":
          return {
            ...
            refreshButton: getRefreshButtonModel(),
            todayButton: getTodayButtonModel(date),
            settingsButton: getSettingsButtonModel(),
            weeklyButtons: getWeeklyButtonsModel(date),
            ribbon: getRibbon(currentTags),
          };
        case "draw":
        ...
        case "draw-weeks":
          const newWeekElements = elements.map((calendarElement) => {
            ...
            return utils.groomedElementModel(calendarElement, !!weekJot, weekJot && weekJot.hasTasks);
          });
        ...
        case "draw-month":
          const monthJot = getMonthJot(date);
          const buttonGraphicsContainer = utils.newHTMLElement("div", undefined, "graphics-container", [utils.getMaterialIcon("description", "mirror-icon")]);
          const monthButton = utils.newHTMLElement("div", undefined, "smol-button notepad", [buttonGraphicsContainer]);
          const newButtonContent = utils.getElementModel(monthButton.firstChild, !!monthJot, monthJot && monthJot.hasTasks);
