(() => {
  ...
  function getState() {
    const lists = Object.keys(pluginState);
    //lists.reduce((resultObj, listName) => ({...resultObj, `${listName}`: [...pluginState[listName]]));
    return {};
  }

  function setState(paramState) { //TODO implement deep copy
    let newState = { ...pluginState };
    ["jots", "weekJots", "monthJots"].forEach((field) => {
    ...
    function redrawCalendar(fpInstance) {
      ...
      window.callAmplenotePlugin("refresh", displayedMonthsDate).then(({ monthlyButton, refreshButton, todayButton, settingsButton, weeklyButtons, ribbon }) => {
        ...
        replaceHTMLElement("#ribbon", ".calendar-container", () => {
          return newHTMLElement(ribbon, (event) => {
            if (event.target.className === "tag-chip") handleSettings(fpInstance);
          });
        }, append);
      });
    };
