(() => {
  ...
  function getState() {
    return {
      ...
      peekTasks: [...pluginState.peekTasks],
    };
  }
  ...
  function setState(newState) {
    ...
    if (newState.peekTasks) pluginState.peekTasks = [...newState.peekTasks];

    // Compare states to detect changes
    const hasChanges = ['jots', 'weekJots', 'monthJots', 'peekTasks'].some(key => {
  ...
  async function fetchPeekTasksPromise(app, domainId) {
    //I want to exclude tasks whose createdAt and startTime does not fit within the current and adjacent months,
    //based on timestamp difference.
    return app.getTaskDomainTasks(domainId);
  }
  ...
  function filterRecentTasks(tasks) {
    const now = new Date();
    // 7 days before the first day of the current month, no ms
    const start = new Date(now.getFullYear(), now.getMonth(), 1 - 7).getTime() / 1000;
    // 14 days after the end of the current month, no ms
    const end = new Date(now.getFullYear(), now.getMonth() + 1, 14).getTime() / 1000;

    return tasks.filter(({ startAt, createdAt, completedAt, dismissedAt }) => {
      //replace this evaluation with a reduce from an array of fieldNames.
      //Same conclusion as current implementation can be reached with lazy evaluation. (acc, cur) => (acc && cur)
      return (completedAt ? false :
        (dismissedAt ? false :
          (startAt ? (startAt >= start && startAt <= end) :
  ...
  async function getActiveDomainName(app) {
    ...
    const currentDomainId = app.settings[pluginConstants.settings.DOMAIN];
    const currentDomainObject = domains.find((domain) => domain.uuid === currentDomainId);
    const currentDomainName = currentDomainObject ? currentDomainObject.name : "task"; //TODO make this "task" a constant?
  ...
  /**
   * This object defines utility functions: technical calculations or reusable bulding blocks.
   */
  var utils = {
  ...
  /** 
   * This object defines functions that:
   * 1. directly modify the DOM of the embedded html page OR
   * 2. call window.callAmplenotePlugin during their execution. 
   * These functions will eventually be imported as strings within the rendered embed content.
   */
  var helpers = () => {

    function newHTMLElement({ tagName, id, className, children }, onClickHandler, displaysTasks) {
      ...
      displaysTasks && window.makeHoverable(newElement);
    ...
    function redrawCalendar(fpInstance) {
      ...
      const [prepend, append, displayTasks] = [true, false, true];

      window.callAmplenotePlugin("refresh", displayedMonthsDate).then(({ monthlyButton, todayButton, weeklyButtons, ribbon, peekTasks }) => {
        fpInstance.redraw();
        ...
        /** TODO draw the default look of the peek section
        replaceHTMLElement("#peek", ".peek", () => {
          return newHTMLElement(ribbon, (event) => handleSettings(fpInstance));
        }, append); */
      });
    };
  ...
  /** This object defines the handlers for the flatpicker instance hooks.*/
  var handlers = {
    getWeek: (date) => {
  ...
  var plugin = {
    ...
    appOption(app) {
      ...
      app.openSidebarEmbed(0.69, fpConfig, pluginHelpers, pluginStyle);
    },
    async onEmbedCall(app, callType, date, shiftPressed, dayName) {
      const pluginConstants = this._constants;

      const [currentFDOW, currentTags, currentSource, currentDomainId] = Object.values(pluginConstants.settings).map((setting) => (app.settings[setting]));
      switch (callType) {
        case "settings":
          ...
          settingsPrompt.inputs[3].value = currentDomainId;
          ...
          if (userSettings) {
            ...
            const fetchIsNeeded = (currentTags !== tags) || (currentSource !== taskSource) || (taskSource === pluginConstants.taskSource.DOMAIN && currentDomainId !== domainId);
        ...
        case "fetch":
          ...
          return Promise.all([
            ...
            fetchPeekTasksPromise(app, currentDomainId).then((tasks) => {
              console.log("FETCHED TASKS", currentDomainId, tasks);
              const filteredTasks = filterRecentTasks(tasks);
              console.log("FILTERED TASKS", filteredTasks);
              //TODO assemble the tasks map
              //score, content, noteUUID, startAt, endAt, allDay?
              return filteredTasks.map(({ content, noteUUID }) => ({ content, noteUUID }));
            }),

          ]).then(([dailyNotes, weeklyNotes, monthlyNotes, peekTasks]) => {
            console.log("parsed tasks", peekTasks);
        ...
        case "refresh":
          return {
            ...
            peekTasks: getState().peekTasks, //TODO
          };
        case "draw":
    ...
    function makeHoverable(element) {
      element.addEventListener("mouseenter", (event) => {
        ...
        hoverTimer = setTimeout(() => {
          //showTasks(day);//TODO
          console.log("Displaying tasks for", event.target.textContent);
        }, 300);
      });
