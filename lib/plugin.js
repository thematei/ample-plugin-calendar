(() => {
  ...
  const pluginConstants = {
    ...
    settings: {
      ...
      DOMAINS: "Domain Id (pick from settings menu)",
    },
    settingsReport: {
      FETCH: "changed",
      REDRAW: "configured",
  ...
  async function updateSettingsPromise(app, firstDayOfWeek, tags, taskSource, taskDomains) {
    let promises = [];
    ...
    if (taskDomains) promises.push(app.setSetting(pluginConstants.settings.DOMAINS, taskDomains));
  ...
  async function getUserDomainOptions(app) {
    ...
    const domains = taskDomains.map(({ name, notes, uuid }) => {
  ...
  function getSettingsButtonModel(taskSource) {
    //Pick icon based on parameter
    const settingsIconName = taskSource === pluginConstants.taskSource.JOTS ? "stylus" : "date_range";
    //Generates the settings button model
    const settingsButtonModel = utils.getElementModel("div", "settings", "settings", [utils.getMaterialIcon(settingsIconName)]);
  ...
  function getRibbon(userTags, taskSource) {
    ...
    const settingsButtonModel = getSettingsButtonModel(taskSource);
  ...
  var helpers = () => {
    ...
    function newHTMLElement({ tagName, id, className, children }, onClickHandler) {
      ...
      children.forEach((child) => {
        if (typeof (child) === "object") newElement.append(newHTMLElement(child, onClickHandler));
        else {
          //Add the onClickHandler only if the element is at the bottom of its DOM hierarchy
          newElement.append(child);
          onClickHandler && newElement.addEventListener("click", (...args) => onClickHandler(...args));
        }
      });
      return newElement;
    };
  ...
  var plugin = {
    ...
    validateSettings(app, settings) {
      ...
      //TODO validate taskSource and taskDomains
    ...
    async onEmbedCall(app, callType, date, shiftPressed, dayName) {
      ...
      switch (callType) {
        case "settings":
          ...
          const domains = await getUserDomainOptions(app);
          console.log("Got uuids", domains, "with selected", currentDomain);
          settingsPrompt.inputs[3].options = domains;
        ...
        case "refresh":
          return {
            ...
            ribbon: getRibbon(currentTags, currentSource),
          };
        case "draw":
