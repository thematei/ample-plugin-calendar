const plugin = {
  ...
  _constants: {
    ...
    settingsPrompt: {
      inputs: [
        { label: "Weekdays", type: "select", options: [{ label: "Sunday", value: 0 }, { label: "Monday", value: 1 }, { label: "Tuesday", value: 2 }, { label: "Wednesday", value: 3 }, { label: "Thursday", value: 4 }, { label: "Friday", value: 5 }, { label: "Saturday", value: 6 },] },
  ...
  async onEmbedCall(app, callType, date, shiftPressed, dayName) {
    ...
    if (callType === "settings") {
      const settings = await app.prompt("Settings", this._constants.settingsPrompt);

      if (settings) {
        const [firstDayOfWeek, tags] = settings;
        return { success: true, firstDayOfWeek, tags };
      } else {
        //fallback settings for now
        //TODO implement revert to initial settings
        return { success: false, firstDayOfWeek: 1, tags: "daily-jot" };
      }
    }
